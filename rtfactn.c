...
int
ecApplyPropChange(RTF_Context *ctx, IPROP iprop, int val)
{
    char *pb;
...
int
ecParseSpecialProperty(RTF_Context *ctx, IPROP iprop, int val)
{
    switch (iprop)
    {
...
int
ecTranslateKeyword(RTF_Context *ctx, char *szKeyword, int param, bool fParam)
{
    int isym;
...
int
ecChangeDest(RTF_Context *ctx, IDEST idest)
{
    if (ctx->rds == rdsSkip)             /* if we're skipping text, */
        return ecOK;                     /* don't do anything */
...
int
ecEndGroupAction(RTF_Context *ctx, RDS rds)
{
    switch (ctx->rds) {
    case rdsTitle:
        if (ctx->title)
...
int
ecParseSpecialKeyword(RTF_Context *ctx, IPFN ipfn)
{
